...
const fields = [['name', 'Company name'], ['sector', 'Sector'], ['region', 'Region'], ['district', 'District'], ['tradeArea', 'Trade area'], ['town', 'Town'], ['location', 'Address / location'], ['partnerType', 'Partner type'], ['operatingModel', 'Operating model'], ['locationVerificationNotes', 'Location verification notes'], ['ghanaPostGps', 'GhanaPost GPS address'], ['contactPerson', 'General contact person'], ['contactPhone', 'General phone'], ['contactEmail', 'General email'], ['website', 'Website'], ['totalSlots', 'Total placement capacity'], ['programs', 'Eligible programmes (comma separated)']] as const
const labels: Record<string, string> = Object.fromEntries(fields)
const fieldOptions: Record<string, { value: string; label: string }[]> = {
  partnerType: [{ value: 'RegisteredCompany', label: 'Registered company' }, { value: 'MasterCraftPerson', label: 'Master Craft Person (MCP)' }, { value: 'Government', label: 'Government' }, { value: 'NGO', label: 'NGO' }, { value: 'Other', label: 'Other' }],
  operatingModel: [{ value: 'FixedSite', label: 'Fixed workshop' }, { value: 'HomeBased', label: 'Home-based' }, { value: 'MobileField', label: 'Mobile / field work' }, { value: 'MultipleSites', label: 'Multiple worksites' }, { value: 'TemporarySite', label: 'Temporary / project site' }, { value: 'NoFixedPremises', label: 'No fixed premises' }],
}
