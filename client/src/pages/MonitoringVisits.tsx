...
import { MoreHorizontal, Download, ShieldCheck, ShieldAlert, ShieldQuestion, AlertTriangle, Handshake } from "lucide-react"
import {
    Dialog,
    DialogContent,
    DialogDescription,
...
export default function MonitoringVisits() {
    ...
    const handleDelete = async (id: string) => {
        if (confirm("Are you sure you want to delete this visit?")) {
            try {
                const res = await authFetch(`/api/monitoring-visits/${id}`, { method: 'DELETE' })
                const payload = await res.json().catch(() => ({}))
             ...
             <div className="flex flex-col md:flex-row items-start md:items-center justify-between space-y-4 md:space-y-2 px-4 sm:px-0">
                ...
                <div className="flex items-center space-x-4">
                    ...
                    <Dialog open={Boolean(gpsReviewVisit)} onOpenChange={(next) => { if (!next) setGpsReviewVisit(null) }}>
                        <DialogContent className="sm:max-w-[760px] max-h-[90vh] overflow-y-auto bg-white border-gray-200 text-gray-900 [&>button]:bg-gray-100 [&>button]:text-gray-600 [&>button]:hover:bg-gray-200 [&>button]:hover:text-gray-900">
                            <DialogHeader>
                                <DialogTitle className="text-gray-900">GPS Review and Evidence</DialogTitle>
                            ...
                            {gpsReviewVisit ? (
                                <div className="space-y-4">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <DocumentUpload
                                            monitoringVisitId={gpsReviewVisit._id}
                                            categories={['Visit Photo', 'Report', 'Other']}
                                            defaultCategory="Visit Photo"
                                            onUploadSuccess={() => loadGpsDocuments(gpsReviewVisit._id)}
                                        />
                                    ...
                                    {(isAdmin || user?.role === 'Admin') ? (
                                        <div className="space-y-3 rounded-2xl border border-gray-200 bg-gray-50 p-4">
                                            ...
                                            <Textarea
                                                value={gpsReviewComment}
                                                onChange={(e) => setGpsReviewComment(e.target.value)}
                                                placeholder="Add review notes, rejection reason, or conditions for the GPS exception."
                                                className="min-h-[120px] bg-white"
                                            />
                                            <div className="flex flex-wrap gap-2">
                                                {gpsReviewVisit.locationVerified === 'Verified' ? (
                                                    <Button disabled={gpsDecisionSubmitting} onClick={() => handleGpsDecision('mark_verified')} className="rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white">
                                                        Mark Verified
                                                    </Button>
                                                ) : null}
                                                <Button disabled={gpsDecisionSubmitting} onClick={() => handleGpsDecision('approve_exception')} className="rounded-xl bg-blue-600 hover:bg-blue-700 text-white">
                                                    Approve Exception
                                                </Button>
                                                <Button disabled={gpsDecisionSubmitting} onClick={() => handleGpsDecision('reject')} variant="outline" className="rounded-xl border-rose-200 text-rose-700 hover:bg-rose-50">
                                                    Reject
                                                </Button>
                                            </div>
                                        </div>
